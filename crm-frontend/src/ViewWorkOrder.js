...
function toDatetimeLocalValue(dateLike) {
  ...
  // Accept multiple formats commonly returned by APIs / MySQL
  const m = moment(dateLike, [moment.ISO_8601, "YYYY-MM-DD HH:mm:ss", "YYYY-MM-DD HH:mm", "YYYY-MM-DD"], true);

  // If strict parse failed, try a non-strict parse as a fallback (covers odd strings)
  const mm = m.isValid() ? m : moment(dateLike);
...
export default function ViewWorkOrder() {
  ...
  // ✅ Track whether we’ve initialized the quick schedule input from backend at least once
  const quickScheduleInitializedRef = useRef(false);
  ...
  const fetchWorkOrder = async () => {
    try {
      ...
      // ✅ FIX: Initialize / refresh quick schedule field safely
      // If backend scheduledDate is in a format that doesn't parse, DO NOT overwrite the input to ""
      const parsed = toDatetimeLocalValue(data?.scheduledDate || "");

      // First load: set it once, even if blank
      if (!quickScheduleInitializedRef.current) {
        quickScheduleInitializedRef.current = true;
        setQuickScheduledDate(parsed || "");
      } else {
        // After that: only update the UI if we have a valid parsed value,
        // OR backend is truly empty (meaning schedule was cleared)
        if (parsed) {
          setQuickScheduledDate(parsed);
  ...
  const fetchResidentialContract = async () => {
    try {
      ...
      setResidentialContract(row);
      setContractDraft({
        scopeOfWork: row?.scopeOfWork ?? "",
        contractTotal:
          row?.contractTotal != null && row.contractTotal !== ""
            ? String(row.contractTotal)
            : "",
        downPaymentPercent:
      ...
      console.error("Error fetching residential contract:", err);
      setResidentialContract(null);
    }
  };

  const saveResidentialContract = async () => {
    setContractSaving(true);
    try {
      const payload = {
        scopeOfWork: contractDraft.scopeOfWork || null,
        contractTotal: contractDraft.contractTotal === "" ? null : Number(contractDraft.contractTotal),
        downPaymentPercent:
          contractDraft.downPaymentPercent === "" ? null : Number(contractDraft.downPaymentPercent),
        startDate: contractDraft.startDate || null,
        completionDate: contractDraft.completionDate || null,
      };
      const res = await api.put(`/work-orders/${id}/residential-contract`, payload, {
        headers: authHeaders(),
      });
      setResidentialContract(res.data || null);
    } catch (err) {
      console.error("Error saving residential contract:", err);
      alert(err?.response?.data?.error || "Failed to save residential contract.");
    } finally {
  ...
  const saveQuickSchedule = async (datetimeLocalVal) => {
    ...
    // Allow clearing schedule by deleting the value
    if (!val) {
      ...
      try {
        ...
        // Immediately reflect cleared state locally to prevent flicker
        setQuickScheduledDate("");
    ...
    setScheduleSaving(true);
    try {
      // ✅ FIX: Send MySQL-friendly string instead of ISO Zulu
      const mysql = datetimeLocalToMySQL(val);
      if (!mysql) throw new Error("Invalid date/time selected.");

      const form = new FormData();
      form.append("scheduledDate", mysql);
      ...
      // ✅ Keep what user chose in the input immediately (prevents “snap back”)
      setQuickScheduledDate(val);

      await fetchWorkOrder();
    } catch (err) {
      console.error("⚠️ Error saving scheduled date:", err);
      alert(err?.response?.data?.error || err?.message || "Failed to save scheduled date.");

      // Revert UI to last known saved value (from workOrder)
      const revert = toDatetimeLocalValue(workOrder?.scheduledDate || "");
      if (revert || !workOrder?.scheduledDate) setQuickScheduledDate(revert || "");
    } finally {
      setScheduleSaving(false);
    }
  };
...
<body>
  ...
  const handleSaveEdits = async () => {
    ...
    try {
      ...
      // ✅ FIX: send MySQL-friendly datetime if using datetime-local
      if ((edit.scheduledDate || "").includes("T")) {
        const mysql = datetimeLocalToMySQL(edit.scheduledDate);
        form.append("scheduledDate", mysql || "");
      } else {
  ...
  // Open the "Send to Customer" compose modal, preselecting the clicked estimate PDF.
  const openEstimateSend = async (preselectPdfId) => {
    try {
      const res = await api.get(`/work-orders/${id}/estimate-send/draft`, { headers: authHeaders() });
      const d = res.data || {};
      const estIds = Array.isArray(d.estimatePdfs) ? d.estimatePdfs.map((e) => e.id) : [];
      ...
      console.error("Error building estimate draft:", err);
      alert(err?.response?.data?.error || "Failed to open estimate composer.");
    }
  };

  const closeEstimateSend = () => setEstSendModal(null);
  ...
  const sendEstimateEmail = async () => {
    ...
    try {
      const res = await api.post(
        `/work-orders/${id}/estimate-send`,
        {
          to: estSendModal.to.trim(),
          subject: estSendModal.subject,
          body: estSendModal.body,
          estimatePdfIds: estSendModal.selectedEst,
          photoIds: estSendModal.selectedPhotos,
          includeSignoff: estSendModal.includeSignoff,
        },
        { headers: authHeaders() }
      );
      const sentTo = res.data?.sentTo || estSendModal.to.trim();
      setEstSendModal(null);
      await fetchEstimateSends();
      await fetchWorkOrder(); // refresh estimateSentAt
      alert(`Estimate sent to ${sentTo}.`);
    } catch (err) {
  ...
  return (
    <div className="view-container">
      ...
      {estSendModal && (() => {
        ...
        const attachCount = m.selectedEst.length + m.selectedPhotos.length + (m.includeSignoff ? 1 : 0);
        const upd = (patch) => setEstSendModal((s) => ({ ...s, ...patch }));
        return (
          <div
                ...
                {editMode ? (
                  ...
                  <div className="schedule-inline">
                    <input
                      ...
                      onChange={(e) => {
                        // ✅ UI only while user is choosing
                        setQuickScheduledDate(e.target.value);
                      }}
                      onBlur={(e) => {
                        // ✅ Save once when user is done
                        saveQuickSchedule(e.target.value);
                      }}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
              ...
              {estimatePdfs.map((pdf) => {
                ...
                return (
                  ...
                  >
                    ...
                    {(() => {
                      const lastSend = estimateSends.find((s) => s.estimatePdfId === pdf.id) || null;
                      return lastSend ? (
